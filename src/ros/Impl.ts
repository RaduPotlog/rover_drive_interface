...
export class Impl {
    ...
    constructor(ws: IWebSocket) {
        ...
        // page opens a native WebSocket to the same-origin /ws endpoint (nginx proxy).
    ...
    async sendServiceRequest<Request, Response>(name: string, request: Request) {
        ...
        return new Promise<Response>((resolve, reject) => {
            const detach = () => {
                ...
                this.#client.off('serviceCallFailure', failure);
            };
            ...
            // The bridge reports a call it could not complete (service gone, timed out
            // on the ROS side); without this the caller would only ever see its own timeout.
            const failure = (event: ServiceCallFailure) => {
                if (event.serviceId === service.id && event.callId === callId) {
                    detach();
            ...
            this.#client.on('serviceCallFailure', failure);
